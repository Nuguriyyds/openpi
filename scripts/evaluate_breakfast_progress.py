"""Offline evaluation and visualization for breakfast progress checkpoints.

The script evaluates every requested checkpoint on the fixed five-breakfast
held-out split and writes complete, independent reports. It deliberately does
not rank or select a checkpoint. Pass ``--export-mp4`` to additionally render
one annotated MP4 per subtask.
"""
...
LOGGER = logging.getLogger("breakfast_progress_evaluation")

DEFAULT_CONFIG_NAME = "pi05_agilex_breakfast_progress"
DEFAULT_EXP_NAME = "breakfast_progress_v1"
DEFAULT_HF_LEROBOT_HOME = Path("/mnt/data/dataset/ei/huggingface")
DEFAULT_CHECKPOINT_BASE = Path("/mnt/data/models/openpi/checkpoints")
...
DEFAULT_CHECKPOINT_STEPS = ("20000", "30000", "latest")
...
def _resolve_checkpoint_steps(checkpoint_root: Path, requested: list[str]) -> list[int]:
    ...
    if len(resolved) < 2:
        LOGGER.warning("Only one unique checkpoint will be evaluated: %s", resolved)
    return resolved


def _frame_metrics(predictions: np.ndarray, targets: np.ndarray) -> dict[str, float | int]:
    ...
    return {
        ...
        "out_of_range_rate": float(np.mean((predictions < 0.0) | (predictions > 1.0))),
    }
...
def _episode_metric(
    ...
    *,
    ...
    result: dict[str, Any] = {
        ...
        **_frame_metrics(predictions, targets),
    }
    differences = np.diff(predictions)
    violation_count = int(np.sum(differences < 0.0))
    true_crossing = _first_crossing(targets, threshold)
    predicted_crossing = _first_crossing(predictions, threshold)
    pre_completion = targets < threshold
    false_positive_count = int(np.sum((predictions >= threshold) & pre_completion))

    result.update(
        {
            "monotonic_comparison_count": int(len(differences)),
            "monotonic_violation_count": violation_count,
            "monotonic_violation_rate": float(violation_count / len(differences)) if len(differences) else 0.0,
...
def _aggregate_episode_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    comparisons = sum(int(row["monotonic_comparison_count"]) for row in rows)
    violations = sum(int(row["monotonic_violation_count"]) for row in rows)
    ...
    return {
        ...
        "monotonic_violation_rate": float(violations / comparisons) if comparisons else 0.0,
...
def compute_metrics(prediction_file: Path, *, fps: float, threshold: float) -> dict[str, Any]:
    ...
    if len(set(row_counts.values())) != 1 or not len(predictions):
        raise ValueError(f"Prediction arrays have invalid row counts: {row_counts}")
    for name, values in (("prediction", predictions), ("target", targets), ("infer_ms", infer_ms)):
        if not np.all(np.isfinite(values)):
            raise ValueError(f"{name} contains non-finite values")
    if np.any((targets < 0.0) | (targets > 1.0)):
        raise ValueError("Target progress values must stay within [0, 1]")
    ...
    for episode_index in sorted(np.unique(episode_indices).tolist()):
        ...
        ordered_targets = targets[mask][order]
        target_is_valid = (
            np.isclose(ordered_targets[0], 0.0)
            and np.isclose(ordered_targets[-1], 1.0)
            and not np.any(np.diff(ordered_targets) < -1e-7)
        )
        if not target_is_valid:
            raise ValueError(f"Episode {episode_index}: target progress is not monotonic from 0 to 1")
        episode_rows.append(
            _episode_metric(
                int(episode_index),
                int(episode_task_indices[0]),
                predictions[mask][order],
                ordered_targets,
                fps=fps,
                threshold=threshold,
            )
    ...
    for task_index in sorted(np.unique(task_indices).tolist()):
        ...
        per_task[str(int(task_index))] = {
            **_frame_metrics(predictions[frame_mask], targets[frame_mask]),
...
def _fixed_noise(seed: int, episode_index: int, frame_index: int, horizon: int, action_dim: int) -> np.ndarray:
    seed_sequence = np.random.SeedSequence([seed, episode_index, frame_index])
    return np.random.default_rng(seed_sequence).standard_normal((horizon, action_dim), dtype=np.float32)
...
def _evaluate_checkpoint_worker(args: argparse.Namespace) -> int:
    ...
    for episode_index in split.TEST_EPISODE_INDICES:
        start = int(episode_from[episode_index])
        end = int(episode_to[episode_index])
        frame_specs.extend(
            (episode_index, local_frame_index, start + local_frame_index)
            for local_frame_index in range(end - start)
        )

    LOGGER.info(
        "Evaluating %s held-out episodes (%s frames), batch_size=%s, denoising_steps=%s",
        len(split.TEST_EPISODE_INDICES),
        len(frame_specs),
        args.batch_size,
        args.num_steps,
    )
    ...
    for batch_start in range(0, len(frame_specs), args.batch_size):
        ...
        for episode_index, local_frame_index, dataset_index in valid_specs:
            ...
            target = float(np.asarray(sample["progress"]).reshape(-1)[0])
        ...
        while len(transformed_items) < args.batch_size:
            ...
            noises.append(np.array(noises[-1], copy=True))
        ...
        noise_batch = jnp.asarray(np.stack(noises, axis=0))
        started = time.monotonic()
        normalized_actions = policy._sample_actions(  # noqa: SLF001
            jax.random.key(args.seed),
            observation,
            noise=noise_batch,
            num_steps=args.num_steps,
        )
        normalized_actions = np.asarray(jax.block_until_ready(normalized_actions))
        ...
        for batch_index, (episode_index, task_index, local_frame_index, target) in enumerate(batch_metadata):
            output = policy._output_transform(  # noqa: SLF001
                {
                    "state": np.asarray(batched_inputs["state"][batch_index]),
                    "actions": normalized_actions[batch_index],
                }
            )
            progress_chunk = np.asarray(output["progress"], dtype=np.float32)
            if progress_chunk.shape != (config.model.action_horizon,):
                raise ValueError(f"Unexpected progress chunk shape: {progress_chunk.shape}")
            ...
            result_predictions.append(float(progress_chunk[0]))
...
def _format_dataset_path(pattern: str, episode_index: int, chunk_size: int, video_key: str | None = None) -> Path:
    return Path(
        pattern.format(
...
def _load_report_series(
    ...
    *,
    ...
    for episode_index in split.TEST_EPISODE_INDICES:
        ...
        if episode_index not in metrics_by_episode:
            raise ValueError(f"Episode {episode_index}: metrics are missing")
        episode_task_indices = np.unique(task_indices[mask])
        if len(episode_task_indices) != 1:
        ...
        if task_index not in tasks:
            raise ValueError(f"Episode {episode_index}: task {task_index} is missing from metadata")
        ...
        series.append(
            {
                "episode_index": episode_index,
                "source_episode_index": episode_index // split.SUBTASKS_PER_SOURCE_EPISODE,
                "subtask_index": episode_index % split.SUBTASKS_PER_SOURCE_EPISODE,
                "task_index": task_index,
                ...
                "prediction": predictions[mask][order].astype(float).tolist(),
...
def _html_document(manifest: dict[str, Any]) -> str:
    ...
    template = textwrap.dedent(
        ...
        <head>
          ...
          <title>Breakfast progress evaluation</title>
          <style>
            :root {
              color-scheme: dark;
        ...
        <body>
          <main>
            <h1>Checkpoint <span id="checkpoint"></span> | breakfast progress</h1>
            <p class="subtitle">Held-out subtasks only. Top camera and raw progress predictions.</p>
            <div class="toolbar">
              <div class="field">
                <label for="episode-select">Subtask episode</label>
            ...
            <section class="viewer">
              ...
              <aside class="details">
                ...
                <div class="live">
                  ...
                  <div><span>Target</span><strong id="target-value">0.000</strong></div>
                  <div><span>Prediction</span><strong id="prediction-value">0.000</strong></div>
                </div>
            ...
            <section class="plot-wrap">
              <div class="legend">
                <span style="--line:#66d9a5">Target progress</span>
                <span style="--line:#ffad5a">Predicted progress</span>
                <span style="--line:#b49cff">0.95 threshold</span>
              </div>
              <canvas id="plot" aria-label="Target and predicted progress by frame"></canvas>
          ...
          <script>
            ...
            function showMetrics(episode) {
              ...
              addMetric("MAE", formatNumber(metrics.mae, 4));
              addMetric("RMSE", formatNumber(metrics.rmse, 4));
              addMetric("Bias", formatNumber(metrics.bias, 4));
              addMetric("Maximum absolute error", formatNumber(metrics.max_abs_error, 4));
              addMetric("Out-of-range rate", formatNumber(metrics.out_of_range_rate * 100, 2) + "%");
              addMetric("Monotonic violation rate", formatNumber(metrics.monotonic_violation_rate * 100, 2) + "%");
              addMetric("Early 0.95 trigger", metrics.early_triggered ? "yes" : "no");
              addMetric("Never reached 0.95", metrics.never_triggered ? "yes" : "no");
              addMetric("Detection delay", metrics.detection_delay_seconds === null
                ? "n/a"
                : formatNumber(metrics.detection_delay_seconds, 3) + " s");
            }
            ...
            function drawPlot(frame) {
              ...
              const allValues = episode.prediction.concat(episode.target);
              const minimum = Math.min(0, Math.min.apply(null, allValues) - 0.04);
              const maximum = Math.max(1, Math.max.apply(null, allValues) + 0.04);
              ...
              const y = function(value) {
                return geometry.bottom - (value - minimum) / Math.max(1e-8, maximum - minimum)
              ...
              drawSeries(episode.prediction, "#ffad5a", 2);
              ...
              context.stroke();
              [["#66d9a5", episode.target[boundedFrame]], ["#ffad5a", episode.prediction[boundedFrame]]]
            ...
            function updateLive(frame) {
              ...
              document.getElementById("target-value").textContent =
                formatNumber(episode.target[currentFrame], 3);
              document.getElementById("prediction-value").textContent =
                formatNumber(episode.prediction[currentFrame], 3);
            ...
            function loadEpisode(index) {
              ...
              document.getElementById("prompt").textContent =
                "Breakfast " + episode.source_episode_index + " | subtask " + (episode.subtask_index + 1)
            ...
            REPORT.episodes.forEach(function(episode, index) {
              ...
              option.textContent = "Breakfast " + episode.source_episode_index
                + " | subtask " + (episode.subtask_index + 1)
                + " | episode " + episode.episode_index;
...
def _make_mp4_plot_base(
    ...
    *,
    ...
    background[:] = (31, 23, 16)
    prediction = np.asarray(episode["prediction"], dtype=np.float64)
    target = np.asarray(episode["target"], dtype=np.float64)
    minimum = min(0.0, float(np.min([prediction.min(), target.min()])) - 0.04)
    maximum = max(1.0, float(np.max([prediction.max(), target.max()])) + 0.04)
    left = 58
    right = width - 18
    top = 62
    bottom = height - 35
    ...
    title = (
        f"Checkpoint {checkpoint_step} | episode {episode['episode_index']} | "
        f"breakfast {episode['source_episode_index']} subtask {episode['subtask_index'] + 1}"
    )
    ...
    cv2.putText(background, "prediction", (116, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (190, 201, 224), 1, cv2.LINE_AA)
    ...
    prediction_points = _plot_points(
        prediction,
        left=left,
        right=right,
        top=top,
    ...
    cv2.polylines(background, [prediction_points], False, (90, 173, 255), 2, cv2.LINE_AA)
...
def _draw_mp4_plot_frame(
    ...
    prediction = np.asarray(episode["prediction"], dtype=np.float64)
    target = np.asarray(episode["target"], dtype=np.float64)
    frame_index = max(0, min(len(prediction) - 1, frame_index))
    ...
    x_value = int(round(left + frame_index / max(1, len(prediction) - 1) * (right - left)))
    ...
    cv2.circle(
        panel,
        (x_value, y_coordinate(float(prediction[frame_index]))),
        4,
        (90, 173, 255),
        -1,
        cv2.LINE_AA,
    ...
    live_text = (
        f"frame {frame_index}/{len(prediction) - 1}  "
        f"target {target[frame_index]:.3f}  prediction {prediction[frame_index]:.3f}"
    )
...
def _comparison_row(step: int, metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    for task_index, task_metrics in sorted(metrics["per_task"].items(), key=lambda item: int(item[0])):
        row[f"subtask_{task_index}_mae"] = task_metrics["mae"]
        row[f"subtask_{task_index}_rmse"] = task_metrics["rmse"]
    return row
...
def _run_evaluation(args: argparse.Namespace) -> Path:
    ...
    if not info_path.is_file() or not episodes_path.is_file():
        raise FileNotFoundError(f"Breakfast progress dataset metadata is incomplete: {dataset_root}")
    dataset_info = _read_json(info_path)
    available_episodes = {int(row["episode_index"]) for row in _read_jsonl(episodes_path)}
    missing_test_episodes = sorted(set(split.TEST_EPISODE_INDICES) - available_episodes)
    if missing_test_episodes:
        raise ValueError(f"Held-out episodes are missing from the dataset: {missing_test_episodes}")
    ...
    LOGGER.info("Held-out episodes: %s", split.TEST_EPISODE_INDICES)
    ...
    for step in steps:
        ...
        checkpoint_summary = {
            ...
            "test_source_episode_indices": list(split.TEST_SOURCE_EPISODE_INDICES),
        ...
        LOGGER.info(
            "Completed checkpoint %s: MAE=%.6f RMSE=%.6f report=%s",
            step,
            metrics["overall"]["mae"],
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--export-mp4",
        action="store_true",
        help="Also export one top-camera-plus-progress MP4 for every held-out subtask and checkpoint.",
    )
...
def main() -> int:
    ...
    try:
        ...
        LOGGER.exception("Breakfast progress evaluation failed")
        return 1
    return 0
