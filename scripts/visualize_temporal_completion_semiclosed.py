...
def _html_document(report: Mapping[str, Any], *, max_episodes: int) -> str:
    ...
    data = {
        ...
        "max_terminal_hold_seconds": float(
            report.get("max_terminal_hold_seconds", report.get("timeout_seconds", 2.0))
        ),
        "max_terminal_hold_ticks": int(
            report.get(
                "max_terminal_hold_ticks",
                report.get("timeout_ticks", round(float(report.get("timeout_seconds", 2.0)) * 2)),
            )
        ),
...
<style>
:root{color-scheme:dark;--bg:#0f172a;--panel:#111827;--grid:#334155;--text:#dbeafe;--target:#61e6b5;--score:#ffab45;--threshold:#b7a2ff;--ref:#60a5fa;--trigger:#fb7185;--arrival:#60a5fa;--switch:#c084fc;--early:#ffab45;--ontime:#61e6b5;--late:#facc15;--missed:#fb7185}
...
<h1>__TITLE__</h1><div class="sub">30 fps gated trajectory replay · 2 Hz rollout · protocol __PROTOCOL__ · terminal hold limit __HOLD_LIMIT__s · threshold __THRESHOLD__ (__SOURCE__) · checkpoint __CHECKPOINT__</div>
<div class="controls"><label>Filter <select id="filter"><option value="all">all</option><option value="all_on_time">all on-time</option><option value="fully_autonomous">fully autonomous</option><option value="has_early">has early</option><option value="has_late">has late trigger</option><option value="has_missed">has missed/stalled</option><option value="not_fully_autonomous">not fully autonomous</option><option value="task0">task 0 failure</option><option value="task1">task 1 failure</option><option value="task2">task 2 failure</option><option value="task3">task 3 failure</option></select></label><label>Episode <select id="episode"></select></label></div>
<div id="cards" class="cards"></div><div class="panel"><div id="info"></div><div id="chart"></div><div class="legend"><span><i class="target"></i>Target at terminal</span><span><i class="score"></i>Completion score</span><span><i class="threshold"></i>Threshold</span><span><i class="arrival"></i>Terminal arrival</span><span><i class="switch"></i>Effective switch</span><span><i class="early"></i>Early</span><span><i class="ontime"></i>On-time</span><span><i class="late"></i>Late trigger</span><span><i class="missed"></i>Missed/stalled</span></div></div>
...
function matches(item){const f=filterEl.value,r=taskResults(item);if(f==='all')return true;if(f==='all_on_time')return !!item.all_on_time;if(f==='fully_autonomous')return !!item.fully_autonomous;if(f==='has_early')return !!item.has_early_switch;if(f==='has_late')return !!item.has_late_trigger;if(f==='has_missed')return !!item.has_missed;if(f==='not_fully_autonomous')return !item.fully_autonomous;if(f.startsWith('task')){const k=Number(f.slice(4));return r.some(x=>x.task_index===k&&x.classification!=='on_time')||((item.not_reached_count||0)>0&&k>=r.length)}return true}
function updateCards(){const s=DATA.summary||{},o=s.overall||{},entries=[['all on-time',s.all_on_time_episode_rate],['fully autonomous',s.fully_autonomous_episode_rate],['early rate',o.early_rate],['on-time rate',o.on_time_rate],['late trigger',o.late_trigger_rate],['missed',o.missed_rate],['not reached',o.not_reached_rate],['prompt mismatch',s.prompt_mismatch_rate]];cardsEl.innerHTML=entries.map(([k,v])=>`<div class="card"><span>${k}</span><b>${fmt(v)}</b></div>`).join('')}
function updateEpisodes(){const items=selected();episodeEl.innerHTML='';items.forEach((x,i)=>{const o=document.createElement('option');o.value=String(i);o.textContent=`episode ${x.full_episode_id} · ${x.all_on_time?'on-time':(x.fully_autonomous?'autonomous':'stalled/early')}`;episodeEl.appendChild(o)});draw()}
...
filterEl.addEventListener('change',updateEpisodes);episodeEl.addEventListener('change',draw);updateCards();updateEpisodes();
    ...
    return (
        ...
        .replace("__HOLD_LIMIT__", f"{data['max_terminal_hold_seconds']:.1f}")
        ...
        .replace("DATA.timeout_ticks", "DATA.max_terminal_hold_ticks")
        .replace("var(--timeout)", "var(--missed)")
        .replace("timeout deadline", "terminal hold limit")
        .replace("timeout ${DATA.timeout_seconds}s", "hold limit ${DATA.max_terminal_hold_seconds}s")
    )
