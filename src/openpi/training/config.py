...
class TrainConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.completion.uses_raw_prefix_completion:
            if self.training_time_rtc.enabled:
                raise ValueError("raw-prefix completion requires the clean non-TTRTC source checkpoint")
            if getattr(completion_head, "variant", None) != "raw_prefix_decoder":
                raise ValueError("raw-prefix completion requires completion_head.variant='raw_prefix_decoder'")
            if self.batch_size != 64:
                raise ValueError("raw-prefix completion requires batch_size=64")
            if self.save_interval != self.completion.val_interval or self.keep_period != self.completion.val_interval:
                raise ValueError(
                    "raw-prefix completion requires save_interval=keep_period=val_interval so every validation "
                    "checkpoint is retained"
                )
            if self.num_train_steps % self.completion.val_interval:
                raise ValueError("raw-prefix completion num_train_steps must end on a validation/checkpoint interval")
            if not isinstance(self.weight_loader, weight_loaders.CheckpointWeightLoader):
                raise ValueError("raw-prefix completion requires CheckpointWeightLoader for the clean backbone")
            expected_params_path = self.completion.raw_prefix_source_checkpoint_path.rstrip("/") + "/params"
            if self.weight_loader.params_path.rstrip("/") != expected_params_path:
                raise ValueError(
                    "raw-prefix completion weight_loader.params_path must match raw_prefix_source_checkpoint_path/params"
                )
            if self.weight_loader.missing_regex != r"completion_head/.*":
...
_CONFIGS = [
    ...
    # Current-only raw-prefix decoder.  The cache and clean source bindings
    # are explicit so a head run cannot silently score a different backbone.
    TrainConfig(
        name="pi05_agilex_breakfast_raw_prefix_completion_head",
        model=pi0_config.Pi0Config(
            pi05=True,
            completion_head=pi0_config.CompletionHeadConfig(
                enabled=True,
